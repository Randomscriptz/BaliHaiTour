...
<form name="formb" action="ChangePWTest.php" method="get">
<?php
   require("../php/routinesTest.php");
   PasswordCheck("");
   // take care of adds
   if (!empty($_GET["UpdateIt"]))
   {
      if (strlen($_GET["NewPassword"]) < 6) die("New password is too short");
      if (strcmp($_GET["NewPassword"],$_GET["NewPasswordB"])) die("The new passwords do not match");
//echo (" compare '" . MD5($_GET["OldPassword"]) . "' '" . $PassRow['PassPassword'] . "' ");
      if (strcmp(MD5($_GET["OldPassword"]),$PassRow['PassPassword'])) die("Old password is wrong");
      //$query = "UPDATE passwords SET PassRecTime=NOW(), PassPassword='" . MD5($_GET['NewPassword']) . "' WHERE PassLoginName='" . $LoginName . "';";
      $MySql = DBOpen();

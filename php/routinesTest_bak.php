<?php
  ...
  function DBOpen()
  {
      global $MySql;
      //echo(" db do connect ");
      $MySql = mysql_connect("balihait_wp743","balihait_main","cM[(UH$r^ho5");
      //echo(" db connected ");
...
//echo('login name = ' . $LoginName . "password = " . MD5($_GET['password']));
      // look up the
      DBOpen();
...
//echo('query = ' . $query);
      ...
      mysql_close($MySql);
//echo (" passwords = '" . $_GET['password'] . "' '" . $PassRow['PassPassword'] . "' ");
      if (!empty($_GET['password']))
      {
//echo (" passwords = '" . $_GET['password'] . "' '" . $PassRow['PassPassword'] . "' ");
         if (strcmp($LoginName,"WEBAccess"))
         {
            if (strcmp(MD5($_GET['password']),$PassRow['PassPassword']))
   ...
   function SchedRead($triptime)
   {
      ...
      if (!($SchedRow = mysql_fetch_array($result)))
         die("Bad TripTime '" . $triptime . "''" . $_GET['TripTime'] . "' ");
   }
 ...
 //printf("costs %s %s ",$SchedRow['SchedAdultCost'],$SchedRow['SchedChildCost']);
      ...
      $MilDiscount = 0;
      $WebDiscount = 0;
      $PremierNumber = $_GET['PremierNumber'];
   ...
   function Finish()
   {
      ...
      ';
      echo('<input type=hidden name=LoginName value="' . $_GET['LoginName'] . '">');
      echo '

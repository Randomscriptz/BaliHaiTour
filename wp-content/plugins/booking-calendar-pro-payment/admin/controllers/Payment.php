...
class wpdevart_bc_ControllerPayments {
	...
	private function send_mail($res_id,$cal_id, $type){
		...
		if(count($extras_data)) {
			...
			$extras .= "<caption style='text-align:left;'>".__('Cost For Passengers:','booking-calendar')."</caption>";
			...
			$extras .= "<tr><td style='padding: 1px 7px;'>".__('Due at trip:','booking-calendar')."</td><td style='padding: 1px 7px;'>".(($reserv["extras_price"]<0)? "" : "+").((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "before") ? esc_html($reserv["currency"]) : '') . $reserv["extras_price"] . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "after") || !isset($this->theme_option['currency_pos'])) ? esc_html($reserv["currency"]) : '')."</td></tr>";
			$extras .= "</table>";
		}

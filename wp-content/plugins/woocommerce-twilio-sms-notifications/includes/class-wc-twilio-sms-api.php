...
class WC_Twilio_SMS_API {
	 ...
	 * @param string $country_code country code in ISO_3166-1_alpha-2 format (WC uses this as standard)
	 * @return string number in e164 format
	 */
	private function format_e164( $number, $country_code ) {

		// if there's no country code, just return the number
		if ( ! $country_code ) {
			return $number;
		}
		...
		// remove international dialing code if entered
		...
		if ( substr( $number, 0, strlen( $country_calling_code ) ) !== $country_calling_code ) {

...
class Clientside_Options {
	...
	static function get_options_sections( $section_slug = '' ) {
		...
		$options_sections = array(
			...
			// Network admin theme
			'clientside-section-network-theme' => array(
				'slug' => 'clientside-section-network-theme',
				'title' => _x( 'Network Theme Options', 'Option section name', 'clientside' ),
				'page' => 'clientside-options-network',
				'options' => array(
					'enable-separators-network'
				)
			),
	...
	static function get_saved_options( $include_user_meta = false, $network = false ) {
		...
		// Special case if this is the network site default settings page
		$force_network_defaults = false;
		if ( is_admin() && isset( $_GET['page'] ) && isset( self::$network_default_pages[ $_GET['page'] ] ) ) {
			$force_network_defaults = true;
		}

		// If not already cached
		if (
			// Force network site defaults
			...
			// Replace defaults with network site defaults if applicable
			if ( is_multisite() ) {
	...
	static function display_form_field_type_image( $field, $value, $name ) {
		...
		<input class="clientside-form-image-input" id="<?php echo 'clientside-formfield-' . $field['name']; ?>" type="text" name="<?php echo esc_attr( $name ); ?>" value="<?php echo $value; ?>">
		<button class="button button-primary clientside-media-select-button" id="<?php echo 'clientside-formfield-' . $field['name']; ?>-upload-button"><?php _ex( 'Select', 'Upload button text', 'clientside' ); ?></button>
		<div class="clear"></div>
		<?php
	}

...
define( 'ADVENTURE_TOURS_VERSION', '2.4.0' );
...
if ( ! function_exists( 'adventure_tours_init_theme_asserts' ) ) {
	...
	function adventure_tours_init_theme_asserts() {
		...
		// wp_register_style( 'jquery-ui-datepicker-custom', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.8.2/themes/smoothness/jquery-ui.css', array(), '1.8.2' );
...
function sv_twilio_sms_variable_replacement( $message, $order ) {
 ...
 // Order tracking
 // $_product = get_post_meta( $order->id, '_product', true ); 
 // $_product = get_post_meta( $order->id, '_product', true ); 
 foreach ($order->get_items() as $key => $lineItem) {
        $products .= ' ' . $lineItem['name'] . '. '; 
        $products .= 'Booking Date: ' . date("d/m/Y", strtotime($lineItem['tour_date']));
    }


//$message = str_replace( '%order_products_test%', 'IT IS WORKING!', $message );
  $message = str_replace( '%order_products%', $products, $message ); 
 // $message = str_replace( '%product%', $_product, $message ); 
